...
string file_1 = "/home/jixingwu/Thirdpart/slam_deepBule/PA6/1/1.png";  // first image
string file_2 = "/home/jixingwu/Thirdpart/slam_deepBule/PA6/1/2.png";  // second image
        ...
        for (int iter = 0; iter < iterations; iter++) {
            ...
            for (int x = -half_patch_size; x < half_patch_size; x++)
                for (int y = -half_patch_size; y < half_patch_size; y++) {
                    ...
                    if (inverse == false) {
                        // Forward Jacobian
                        J[0] = double(GetPixelValue(img2, u2+1, v2) - GetPixelValue(img2, u2-1, v2))/2;
                        J[1] = double(GetPixelValue(img2, u2, v2+1) - GetPixelValue(img2, u2, v2-1))/2;
                        error = double(GetPixelValue(img2, u2, v2) - GetPixelValue(img1, u1, v1));
                    } else {
                        // Inverse Jacobian
                        // NOTE this J does not change when dx, dy is updated, so we can store it and only compute error
                        J[0] = double(GetPixelValue(img1, u1+1, v1) - GetPixelValue(img1, u1-1, v1))/2;
                        J[1] = double(GetPixelValue(img2, u1, v1+1) - GetPixelValue(img1, u1, v1-1))/2;
                        error = double(GetPixelValue(img2, u2, v2) - GetPixelValue(img1, u1, v1));
                    }
